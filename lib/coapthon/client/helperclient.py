...
from lib.coapthon.utils import generate_random_token
...
class HelperClient(object):
    ...
    def _wait_response(self, i,message=None):
        if i==2:
            print "Timed Out---------------------------------------------------------------------- Trying Next Node"
            lib.DataStructures.timed_out_flag=1
            self.queue.put(None)
        elif message.code != defines.Codes.CONTINUE.number:
